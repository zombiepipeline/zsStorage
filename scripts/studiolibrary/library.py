...
class Settings:
    def __init__(self):
        ...
        LibraryWidgetAppdata = os.path.join(os.environ['APPDATA'], 'StudioLibrary')
        if not os.path.exists(LibraryWidgetAppdata):
            os.makedirs(LibraryWidgetAppdata)

        LibraryWidget.data['Default']['path'] = self.project_library

        with open(os.path.join(LibraryWidgetAppdata, 'LibraryWidget.json'), 'w') as f:
            json.dump(LibraryWidget.data, f, indent=2)
    ...
    def getPipe(self):
        data = APP.shotgun.find_one('PipelineConfiguration', [['code', 'is', CONFIG.get_name()]], ['code', 'sg_pipe_path'])
        print(data)
