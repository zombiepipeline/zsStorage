...
class NewTab_UI(QtWidgets.QDialog):
    ...
    def CREATETABS(self):
        ...
        if MAIN_TAB_NAME == "":
            ...
            return
        if SECOND_TAB_NAME == "":
            AssetIt_Global.WarningWindow("You should give a SECOND Tab Name", 250)
            return

        #CREATE MAIN TAB
        try:
            MAIN_TAB_PATH = ASSET_LIBRARY_PATH + "/" + MAIN_TAB_NAME
            os.mkdir(MAIN_TAB_PATH)
        except:
            AssetIt_Global.WarningWindow("MAIN Tab Name already exist, delete it first", 250)
            return
