...
# Instructions:
# Select an object (Do one at a time)
# Run the script.
# Select the correct .ass file from the list
...
def run_replace(delete_source=True, view_mode=6):
    ...
    if not cmds.objExists(duplicate_name):
        ...
        locator = cmds.spaceLocator(p=(0, 0, 0), n=clean_name + "_Loc")
    ...
    if delete_source:
        ...
        delete_appended_name = cmds.rename(clean_name, clean_name.split("|")[-1] + "_DELETE")
